...
<div class="modal-content">
    <div class="modal-body px-5 pb-5">
        ...
        <form id="addRoleForm" method="POST" class="row" enctype="multipart/form-data"
            action="{{route('admin.departamentos.update', $departamento['cod_departamento'])}}">
            @csrf
            @method('PUT')
            @if ($errors->any())
            ...
            <div class="form-group row">
                <div class="mb-1 col-md-4">
                    <label for="register-vicepresidencia"
                        class="form-label">{{__('Vicepresidencies or Directions')}}</label>
                    <select name="direccion_vp" class="form-select">
                        ...
                        <option value="{{$vicepresidencia->cod_vicepresidencia}}"
                            {{($vicepresidencia->cod_vicepresidencia == $departamento['cod_vicepresidencia'] ? 'selected':'' )}}>
                            {{__($vicepresidencia->nombre_vicepresidencia. ' - '.$vicepresidencia->empresa->nombre)}}
                        </option>
                        @endforeach
                    </select>
                    @error('cominidad')
