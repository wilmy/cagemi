    ...
    <div class="col-lg-3 col-sm-6">
      <div class="card">
        <div class="card-body d-flex align-items-center justify-content-between">
          <div>
            <h3 class="fw-bolder mb-75">{{count($DepartamentosXVicepresidencias)}}</h3>
    ...
    <div class="row">
      <div class="col-lg-3 col-sm-6">
        <a href="{{route('admin.departamentos.create')}}">
          <span class="btn btn-primary mb-1">{{__('New departament')}}</span>
        </a>
      </div>
    </div>
  @endcan
      ...
      <table class="user-list-table table">
        <thead class="table-light">
          <tr>
            ...
            @canany(['departamentos.edit'])
              <th>{{__('Action')}}</th>
            @endcan
          </tr>
        </thead>
        <tbody>
          @foreach($DepartamentosXVicepresidencias as $departamento)
            <tr>
              <td>{{ $departamento->nombre_departamento }}</td>
              <td>{{ $departamento->vicepresidencia->nombre_vicepresidencia }}
              </td>
              ...
              @canany(['departamentos.edit'])
                <td>
                  @can('departamentos.edit')
                    <a href="{{route('admin.departamentos.edit', $departamento->cod_departamento)}}" class="btn btn-sm btn-primary role-edit-modal">
